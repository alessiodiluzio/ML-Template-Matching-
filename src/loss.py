...
#     label_true = label
#     label_false = (label - 1) * -1
#     weights = 1/balance_factor * label_true + 1/(1 - balance_factor) * label_false
#     return weights
#
#
# def cross_entropy_loss(logits, label, activation, balance_factor, training=True):
#     cross_entropy = tf.losses.categorical_crossentropy()
#     if activation.lower() == 'sigmoid':
#         tf.compat.v1.nn.softmax_cross_entropy_with_logits_v2(labels=label, logits=logits)
#     if training:
